	...
	return (
		<Layout projectName={data.project.name} projectDescription={data.project.description}>
			<nav className="flex items-center gap-2 text-sm text-gray-500 mb-6 dark:text-gray-400 flex-wrap">
				...
				<span>/</span>
				<span className="text-gray-900 font-medium dark:text-white">Story {story.id}</span>
			</nav>
			...
			<div className="bg-white rounded-lg shadow-sm border p-6 mb-8 dark:bg-gray-800 dark:border-gray-700">
				<div className="flex items-start justify-between gap-4 mb-4">
					<div>
						...
						<p className="text-sm text-gray-500 mt-1 dark:text-gray-400 font-mono">ID: {story.id}</p>
					</div>
					<span
				...
				{story.description && (
					<div className="prose max-w-none dark:prose-invert">
						<p className="text-gray-700 dark:text-gray-300 leading-relaxed">{story.description}</p>
					</div>
				)}
			</div>
			...
			<div className="grid gap-3">
				{story.tasks && story.tasks.length > 0 ? (
							...
							<div className="flex justify-between items-start gap-4">
								<div className="flex-1">
									<div className="flex items-center gap-2 mb-1">
										<span className="text-gray-400 text-xs font-mono dark:text-gray-500">Task {task.id}</span>
										<h3 className="text-lg font-medium text-gray-900 dark:text-white">{task.title}</h3>
									</div>
									{task.description && (
										<p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2 mt-1">{task.description}</p>
									)}
								</div>
								<span
					...
					<div className="text-center py-8 bg-gray-50 rounded-lg border border-dashed dark:bg-gray-800 dark:border-gray-700">
						<p className="text-gray-500 dark:text-gray-400">No tasks defined for this story</p>
					</div>
				)}
			</div>
		</Layout>
