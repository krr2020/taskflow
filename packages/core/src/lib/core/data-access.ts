...
export function loadTaskFile(filePath: string): TaskFileContent | null {
	...
	try {
		// biome-ignore lint/suspicious/noExplicitAny: JSON reading needs any for initial parsing
		...
		if (data.subtasks) {
			consoleOutput(
				`Debug subtasks for ${path.basename(filePath)}: ${JSON.stringify(data.subtasks)}`,
				{ type: "info" },
			);
		}
		...
		consoleOutput(
			`Validation error for ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
			{ type: "error" },
		);
		return null;
	}
