...
import { loadTasksProgress, getTaskFilePath, loadTaskFile } from "./data-access.js";
	...
	private setupRoutes() {
			...
			try {
				...
				if (featureRef.path) {
					try {
						...
						consoleOutput(`Error loading feature details: ${err}`, { type: "error" });
		...
		this.app.get("/api/tasks/:id", (c) => {
			...
			try {
				...
				if (!filePath) {
					consoleOutput(`Task file path not found for ID: ${id}`, { type: "warn" });
					return c.json({ error: "Task file not found" }, 404);
				}

				const taskData = loadTaskFile(filePath);
				if (!taskData) {
					consoleOutput(`Failed to load task file content from: ${filePath}`, { type: "error" });
					return c.json({ error: "Failed to load task data" }, 500);
				}

				return c.json(taskData);
			} catch (e) {
				consoleOutput(`Error loading task ${id}: ${String(e)}`, { type: "error" });
				return c.json({ error: String(e) }, 500);
			}
		});
